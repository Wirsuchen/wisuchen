...
function resetDailyUsageIfNeeded() {
  ...
  if (dailyUsage.date !== today) {
    console.log(`[Cron] Resetting daily usage counter. Previous: ${dailyUsage.count}`)
...
async function translateWithRetry(
  ...
  for (let attempt = 0; attempt < CONFIG.MAX_RETRIES; attempt++) {
    try {
      if (!canMakeRequest()) {
        console.log(`[Cron] Daily limit reached (${dailyUsage.count}/${CONFIG.DAILY_LIMIT})`)
      ...
      if (statusCode === 429 || errorMessage.includes('429') || errorMessage.toLowerCase().includes('rate limit')) {
        console.log(`[Cron] Rate limit hit (429). Attempt ${attempt + 1}/${CONFIG.MAX_RETRIES}`)
...
export async function GET(request: NextRequest) {
  ...
  try {
    ...
    console.log(`[Cron] Checking ${jobs.length} jobs for missing translations...`)
    ...
    for (const job of jobs) {
      ...
      if (translated >= CONFIG.MAX_TRANSLATIONS_PER_RUN) {
        console.log(`[Cron] Reached max translations per run (${CONFIG.MAX_TRANSLATIONS_PER_RUN})`)
        break
      }
      ...
      for (const lang of TARGET_LANGUAGES) {
        ...
        try {
          ...
          const existingTranslation = await getStoredTranslation(contentId, lang, 'job' as ContentType)
          ...
          // Translate title
          const titleResult = await translateWithRetry(job.title || '', lang, 'en')
          ...
          const stored = await storeTranslation(contentId, lang, 'job', translationData)
    ...
    const summary = {
      message: rateLimited ? 'Stopped early due to rate limit' : 'Translation job completed',
