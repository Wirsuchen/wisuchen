...
export class JobSyncService {
  ...
  async syncAllJobs(params?: {
    ...
    try {
      ...
      console.log(`📥 [JobSync] Fetched ${rapidJobs.jobs.length} jobs from RapidAPI`)
      ...
      try {
          ...
          for (const job of response.results) {
            try {
              const normalized = {
                ...
                salary: job.salary_min || job.salary_max ? `€${job.salary_min || 0}-€${job.salary_max || 0}` : undefined,
        ...
        console.log(`📥 [JobSync] Synced ${adzunaResponse[0]?.results.length || 0} jobs from Adzuna`)
      } catch (error) {
  ...
  private async upsertJob(job: any): Promise<'new' | 'updated' | 'skipped'> {
    try {
      ...
      const jobData = {
        ...
        source: job.source,
        external_id: job.id,
        published_at: job.posted_date || job.publishedAt || new Date().toISOString(),
